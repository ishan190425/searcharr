...
class StatusFinder:
    ...
    async def queue(self,update, context):
        ...
        message = ""
        try:
            parts = update.message['text'].split("/status")
            title = parts[1].strip() if len(parts) > 1 else ""
        except (TypeError, AttributeError):
            await update.message.reply_text("Error processing your request.")
            return
        if not title:
            await update.message.reply_text("Please provide a title to search for. Usage: /status <title>")
            return
        ...
        if not message_chunks:
            await update.message.reply_text(f"{title} was not found...")
            return
        else:
    ...
    async def restart(self, update=None, context=None):
        ...
        try:
            ...
            if update and not self._is_admin_user(update):
                await update.message.reply_text("You need admin privileges to restart containers.")
                return False
            ...
            if success:
                ...
                if update:
                    await update.message.reply_text("Container restarted successfully.")
            else:
                self.logger.error(f"Failed to restart container: {process.stderr}")
                if update:
                    await update.message.reply_text(f"Failed to restart container. Error: {process.stderr}")
            ...
            if update:
                await update.message.reply_text(f"Error restarting container: {e}")
            return False
    ...
    async def notify_if_container_down(self, bot):
